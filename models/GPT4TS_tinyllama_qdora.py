...
class Model(nn.Module):
    ...
    def reinit_seqlen_predlen(self, configs): #, device):
        ...
        for layer in (self.head, self.out_layer): #(self.gpt2, self.in_layer, self.out_layer):
            layer.to(device=device)
            layer.train()

        print("later on, self.seq_len: ", self.seq_len, "self.pred_len: ", self.pred_len, " self.patch_num: ",  self.patch_num)
    ...
    def __init__(self, configs): #, device):
        ...
        self.with_inst_norm = configs.with_inst_norm
        ...
        configs.orig_seq_len = configs.seq_len if configs.orig_seq_len == 0 else configs.orig_seq_len
        configs.orig_pred_len = configs.pred_len if configs.orig_pred_len == 0 else configs.orig_pred_len

        self.seq_len = configs.orig_seq_len
        self.pred_len = configs.orig_pred_len
        self.patch_num = (self.seq_len - self.patch_len) // self.stride + 1
        print("self.seq_len: ", self.seq_len, "self.pred_len: ", self.pred_len, " self.patch_num: ",  self.patch_num)
        ...
        configs.d_model = 2048 #4096 #768
        self.d_model =  configs.d_model

        # patching and embedding
        self.patch_embedding = PatchEmbedding( configs.d_model, patch_len, self.stride, padding, configs.dropout)
        ...
        self.encoder = Encoder(
            [
                EncoderLayer(
                    AttentionLayer(
                        FullAttention(True if self.encoder_type == 3 else False, configs.factor, attention_dropout=configs.dropout,
            ...
            norm_layer=torch.nn.LayerNorm(configs.d_model)
                ...
                # Prediction Head
        self.head_nf = configs.d_model * \
                       int((self.seq_len - patch_len) / self.stride + 2)
        if self.task_name == 'long_term_forecast' or self.task_name == 'short_term_forecast':
            self.head = FlattenHead(configs.enc_in, self.head_nf, self.pred_len,
                                    head_dropout=configs.dropout)
        ...
        if 1==1: # configs.is_gpt:
            if 1==1: #configs.pretrain:
                ...
                self.gpt2 = AutoModelForCausalLM.from_pretrained(
                    "TinyLlama/TinyLlama_v1.1", #"TencentARC/LLaMA-Pro-8B", #"meta-llama/Meta-Llama-3-8B", #"TencentARC/LLaMA-Pro-8B",
            ...
            print("before gpt2 = {}".format(self.gpt2))

            self.gpt2.model.model.layers = self.gpt2.model.model.layers[: configs.gpt_layers] #[:6]
        ...
        self.out_layer = nn.Linear(configs.d_model * self.patch_num, self.pred_len) #1024 ) 
